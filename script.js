...
function wireWhatsAppLinks(){
  document.querySelectorAll('a[href^="https://wa.me/YOURNUMBER"]').forEach(a => {
    if(a.closest(".product-card")) return;
...
function setupLightbox(){
  ...
  const openProductDetail = (p, pushHistory) => {
    if(!p) return;
    lightboxImg.src = encodeURI(p.file);
    ...
    document.getElementById("lightboxPrice").textContent = `₹${p.price.toFixed(2)}`;
    document.getElementById("lightboxOrder").href = waLink(p.name, p.file, p.price);

    const parsed = parseSizes(p.sizes);
  ...
  document.addEventListener("click", (e) => {
    ...
    if(!trigger) return;
    const p = PRODUCTS.find(x => x.name === trigger.dataset.name);
    openProductDetail(p, true);
  });

  closeBtn.addEventListener("click", () => close(true));
  document.addEventListener("keydown", (e) => { if(e.key === "Escape" && lightbox.classList.contains("open")) close(true); });
  ...
  window.addEventListener("popstate", () => {
    ...
    if(slug){
      ...
      if(p) openProductDetail(p, false);
    } else {
      close(false);
    }
  });

  // Open directly to a product if the page was loaded with ?product=slug
  const initialParams = new URLSearchParams(window.location.search);
  const initialSlug = initialParams.get("product");
  if(initialSlug){
    const p = PRODUCTS.find(x => slugify(x.name) === initialSlug);
    if(p) openProductDetail(p, false);
  }
}

function setupMiniSlider(){
  ...
  const advance = () => {
    clearTimeout(timer);
    timer = setTimeout(() => { show(current + 1); advance(); }, 4500);
  };
  const restartTimer = () => advance();

  dots.forEach(dot => {
    dot.addEventListener("click", () => { show(parseInt(dot.dataset.dot, 10)); restartTimer(); });
  });
  if(prevBtn) prevBtn.addEventListener("click", () => { show(current - 1); restartTimer(); });
  if(nextBtn) nextBtn.addEventListener("click", () => { show(current + 1); restartTimer(); });
